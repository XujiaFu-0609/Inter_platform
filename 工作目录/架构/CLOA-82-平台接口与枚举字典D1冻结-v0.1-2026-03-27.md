...
4. 依赖项（[CLOA-78](/CLOA/issues/CLOA-78)、[CLOA-79](/CLOA/issues/CLOA-79)）未回稿前，维持 `in_progress` 并持续滚动更新。
...
> 说明（2026-03-27 D1 初稿）：`CLOA-78`、`CLOA-79` 当前状态均为 `todo`，本节先冻结 `CLOA-82` 的**可执行口径**，待两任务回稿后在 v1.1 做最终落章。
...
1. **CLOA-79 回稿未最终落章（高）**：`CLOA-78` 已提交 D1 冻结稿并可并入；当前主要阻塞变更为 `CLOA-79` 控制面口径未回稿。
...
更新时间：2026-03-28 00:31（Asia/Shanghai）
...
3. [CLOA-79](/CLOA/issues/CLOA-79)：`in_progress`，评论线程仍缺“交付文档路径 + 4 点技术摘要 + 未决风险/阻塞”的最小回稿包。
4. [CLOA-82](/CLOA/issues/CLOA-82)：状态转为 `blocked`，在 `CLOA-79` 回稿后 1 小时内发布 v1.1 固化稿并申请转 `in_review`。
5. 截至 2026-03-28 00:31（Asia/Shanghai），`CLOA-79` 仍未回传可并版输入，且已错过 `2026-03-27 23:59` 回稿窗口。

## [v1.1 收口清单]

1. 回稿窗口：**2026-03-27 23:59（Asia/Shanghai）**  
   - [CLOA-78](/CLOA/issues/CLOA-78)：已完成（`done`）。
   - [CLOA-79](/CLOA/issues/CLOA-79)：待提交 `controlPlaneHealth` 判定优先级、CRD->DTO 最小映射、`degraded/down` 阈值。
2. 并版窗口：收到双侧回稿后 **1 小时内**，发布 `CLOA-82 v1.1`。
3. 最终时点：**2026-03-28 12:00（Asia/Shanghai）** 前完成 D1 冻结回执。
4. 升级触发：若 2026-03-27 23:59 仍无最小回稿，将 `CLOA-82` 转为 `blocked` 并在父任务 [CLOA-76](/CLOA/issues/CLOA-76) 标注高风险未收口。
